...
api="c578aba614bdd39a54284af930677a54"
...
#place="Mödling"   # Uncomment and add name or id. NB If the name has spaces, then you must use the id.
...
if [[ -z "$api" ]] &>/dev/null;then
	...
	tempkelv=$(bc -l <<<"${temperature::-3}+273.15")
        #Example format for output:
        #printf "%s; %s; %s %s" "$city" "$temperature" "$windspeed" "$winddir"
        printf "%s: %s\nTemp: \${font sans-serif:bold:size=8}\$color2%s\$color0\${font sans-serif:normal:size=8}=%sK\nHumidity: %s\nWindspeed: %s\nWinddir: %s\nPressure: %s\n" "$city" "$description" "$temperature" "$tempkelv" "$humidity" "$windspeed" "$winddir" "$pressure"
